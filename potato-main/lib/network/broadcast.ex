defmodule Potato.Network.Broadcast do
  ...
  defp handle_disconnect(remote) do
    ...
    Ecto.Query.from(s in Status, where: s.node_id == ^to_string(remote), select: s)                     #DI
      |> Repo.update_all(set: [updated_at: DateTime.utc_now(), sensor_node_hardware_status: "Broken", sensor_node_software_status: "Crashed"])                                                          #SNH
  ...
  defp handle_connect(remote) do
    ...
    if Repo.get_by(Status, node_id: to_string(remote)) == nil do                                        #DI
      ...
      Ecto.Query.from(s in Status, where: s.node_id == ^to_string(remote), select: s)                   #DI
      |> Repo.update_all(set: [updated_at: DateTime.utc_now(), sensor_node_hardware_status: "Working"]) #SNH
    end
