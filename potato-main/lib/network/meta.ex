defmodule Potato.Network.Meta do
  ...
  @doc """
  Sets a logical field in the node descriptor.
  """
  def set_local_nd_field(field, value), do: call(__MODULE__, {:set_local_nd_field, field, value})
  ...
  def handle_call({:set_local_nd_field, field, value}, _from, state) do
    # Update our own ND in the state.
    new_self = Map.put(state.self, field, value)
    new_state = %{state | self: new_self}
    # Notify the network that we have updated our ND.
    broadcast_local_node_descriptor_changed(new_state)
    {:reply, :ok, new_state}
  end
  ...
  def handle_cast({:set_remote_nd_update, remote, map}, state) do
    # If the new ND is not nil, we store it.
    if valid_nd?(map) do
      Logger.debug("Updated remote ND for #{inspect(remote)}")
      new_state = %{state | others: Map.put(state.others, remote, map)}
      Potato.PubSub.call_all(:node_descriptors, {:updated, remote, map})
      {:noreply, new_state}
    else
      Logger.warn("Updated remote ND was not valid: #{inspect(map)}")
      {:noreply, state}
    end
  end
  ...
  defp broadcast_local_node_descriptor_changed(state) do
    abcast(Node.list(), __MODULE__, {:set_remote_nd_update, Node.self(), state.self})
  end
  ...
  defp valid_nd?(nd) do
    Map.has_key?(nd, :hardware) and
      Map.has_key?(nd, :type) and
      Map.has_key?(nd, :name) and
      Map.has_key?(nd, :uuid)
  end
