defmodule SensorNode do
  ...
  def read_measurement() do
    try do
      ...
      failure = true
    Process.sleep(5000)
    %{temperature: Float.ceil(temp, 1), humidity: Float.ceil(humidity, 1), light: Float.ceil(light, 1), noise: noise, motion: (fn v -> if v == 0, do: false, else: true end).(motion), co2: co2, sensor_failure: failure}
  end
